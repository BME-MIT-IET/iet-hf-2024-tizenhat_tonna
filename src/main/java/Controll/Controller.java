...
public class Controller {
        ...
        while (!commandList.isEmpty()) {
            ...
            switch (cmd[0]) {
                ...
                case ("exit"):
                    return;
                default:
                    logger.log(Level.INFO, "Hibás parancs.");
            }
        }
    }
    ...
    public static void Game() {
        while (gameMode) {
            ...
            switch (cmd[0]) {
                ...
                case ("move"):
                    moves++;
                    if (!objectNames.get(cmd[1]).equals(currentPlayer))
                        logger.log(Level.INFO, StringResourceController.WRONG_PLAYER);
                    else move(cmd);
                    break;
                case ("breakfield"):
                    moves++;
    ...
    public static void saboteur(String[] cmd) {
        ...
        SaboteurDraw sd = new SaboteurDraw(0, 0);
        ViewGame.setDrawsNames(sd, tmp);
        ViewGame.setDrawsReverseNames(tmp, sd);
        objectNames.put(cmd[1], tmp);
        objectReverseNames.put(tmp, cmd[1]);
    ...
    public static void mechanic(String[] cmd) {
        ...
        Field f = (Field) objectNames.get(cmd[2]);
        tmp.setStandingField(f);
        MechanicDraw sd = new MechanicDraw(0, 0);
        ViewGame.setDrawsNames(sd, tmp);
        ViewGame.setDrawsReverseNames(tmp, sd);
        String[][] commands = new String[cmd.length - 3][2];
        for (int i = 3; i < cmd.length; i++) {
    ...
    public static void random(String[] cmd) {
        if (test) {
            if (cmd.length == 2) {
                switch (cmd[1]) {
                    case StringResourceController.FALSE:
                        random = false;
                        outResults.add("A véletlen események ki lettek kapcsolva.");
                        break;
                    case "true":
                        random = true;
            ...
            if (cmd.length == 2) {
                switch (cmd[1]) {
                    case StringResourceController.FALSE:
                        random = false;
                        logger.log(Level.INFO, "A véletlen események ki lettek kapcsolva.");
                        break;
                    case "true":
                        random = true;
    ...
    public static void move(String[] cmd) {
        ...
        if (neighbors.contains(f)) {
            ...
            if (test) {
                outResults.add(StringResourceController.WRONG_ACTION);
                lastResult = false;
            } else {
    ...
    private static void neighborsContains(Player p, Field f) {
        if (p.move(f)) {
            if (test) {
                outResults.add(StringResourceController.GOOD_ACTION);
                lastResult = true;
            } else {
